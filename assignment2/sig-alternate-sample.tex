...
\title{CSE255-A Assignment 2 Report}
...
\keywords{data mining, natural language processing}

\section{Explanatory Analysis}
In assignment 2, we use Amazon movie reviews from \url{http://snap.stanford.edu/data/web-Movies.html}. 
...
The basic idea of sentiment analysis is to break the sentence into words, and see if each word is in the sentiment dictionary. Our dictionary is combined with the dictionary downloaded from \url{https://www.cs.uic.edu/~liub/FBS/sentiment-analysis.html}, and the dictionary we created from the training data.

\subsubsection{Build Dictionary}
...
\begin{equation}
\frac{P(w \text{ is positive}|w)}{P(w \text{ is negative}|w)} = \frac{P(w\in r |\text{pos review } r)P(\text{pos})}{P(w\in r |\text{neg review } r)P(\text{neg})}
\end{equation}

And then we pick out the words with this ratio higher than 5.0 or lower than 0.2 and add them to the corresponding sentiment dictionary. Some of the sentiment words we chose through Naive Bayes Classifier are as showed in Table \ref{my-label}. As we can find in Table \ref{my-label}, many words are just movie names or characters' names. For example, Dhampir is a creature half vampire half human, and this word has high neg/pos ratio, so we assume that movies with Dhampir characters are tend to be lame movies.

\begin{table}[]
\centering
\caption{Sentiment words generated}
\label{my-label}
\begin{tabular}{|l|l|l|}
...
After building dictionary, we can easily know whether a word is positive or negative one. We calculate the number of positive words and negative words in a review, and add these two number as features in our linear regression model.

\section{RELATED LITERATURE}
\subsection{NLP model}
When trying to determine the natural language processing process, we mainly use sentiment analysis and opinion mining to get the 
{\secit mining and summarizing customer reviews}

We use the similar sentiment dictionary from this paper. And when adopting the algorithm mentioned in this paper, we revised it into a simplified one, you can see more detailed description of the algorithm in the previous chapter.
